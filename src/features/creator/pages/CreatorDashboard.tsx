...
export const CreatorDashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      // El autocompletado ahora nos asegura que data tiene .creator y .goals
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {posts.length === 0 ? (
            <div className="bg-white p-12 rounded-2xl border border-dashed border-gray-300 text-center">
              <p className="text-gray-500">Aún no has publicado nada. ¡Empieza hoy!</p>
            </div>
          ) : (
            posts.map(post => (
              <div key={post.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-50 flex items-center text-sm text-gray-500">
                    ...
                    {post.comments?.length || 0} comentarios (Solo visibles para ti)
                  </div>
                </div>
              </div>
            ))
